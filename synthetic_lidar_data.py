...
def generate_ground_plane(r_max=100.0, n_rings=600, pts_per_ring=40, z_noise=0.02, rng=None):
...
def generate_pole(center_xy, height=2.0, radius=0.08, n_points=120, rng=None):
...
def generate_dynamic_cluster(center_xy, n_points=60, spread=0.35, height=1.7, rng=None):
...
def generate_boundary_stress_ring(radius=10.0, n_points=400, jitter=0.02, rng=None):
    """
...
def build_scene(include_boundary_stress=True, rng=None):
...
def build_moving_sequence(n_frames=10, start_xy=(6.0, -3.0), velocity=(0.4, 0.15), rng=None):
    """
